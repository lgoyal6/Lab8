...
// The same recipe URLs declared in main.js, mirrored here so they can be
// pre-cached when the ServiceWorker installs.
const RECIPE_URLS = [
...
self.addEventListener('install', function (event) {
  event.waitUntil(
    caches.open(CACHE_NAME).then(function (cache) {
      // B6. Add all of the URLs from RECIPE_URLS so they are cached on install
...
self.addEventListener('fetch', function (event) {
  ...
  event.respondWith(
    caches.open(CACHE_NAME).then(function (cache) {
      // B8. If the request is cached, return it. Otherwise fetch it from the
      //     network, add the response to the cache, and return it.
