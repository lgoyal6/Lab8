  ...
  if ('serviceWorker' in navigator) {
    ...
    window.addEventListener('load', () => {
      ...
      navigator.serviceWorker
        .register('./sw.js')
        .then((registration) => {
          // B4. Log that registration was successful
          console.log(
        ...
        .catch((error) => {
          // B5. Log that registration failed
...
async function getRecipes() {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    for (let i = 0; i < RECIPE_URLS.length; i++) {
      // A5. Create a try / catch block to handle the asynchronous code
      try {
        // A6. Fetch the URL
        ...
        recipes.push(recipe);
        // A9. If we've retrieved all of the recipes, save them and resolve
